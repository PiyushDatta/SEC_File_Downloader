...
from src.AppGUI.FileDownloadPanel import FileDownloadsPanel
from src.AppGUI.MainGUIActions import MainGUIActions
from src.AppGUI.MainMenuPanel import MainMenuBar
from src.AppGUI.TopLayerPanel import TopPanel
...
class MainGUIApp(tk.Tk):
    def __init__(self, window_title, window_width, window_length):
        ...
        self.parent_path = os.getcwd() + '\\src'

        # Window settings
        self.title(window_title)
        self.icon = self.parent_path + '\\SEFD_logo_icon.ico'

        # In case someone deletes the ico logo from the src directory
        if os.path.isfile(self.icon) is False:
            self.icon = None
            print(self.icon)
        ...
        self._pickle_file = self.parent_path + "\\CurrentUser.svc"
        self.load_user_details()

        # Set current directory as user's last directory, or parent dir
        self.current_directory = os.path.abspath(os.path.join(self.parent_path, os.pardir))
        ...
        for F in (TopPanel, FileDownloadsPanel):
            ...
            print(page_name)
            # Top Panel/Frame
            if page_name == "src.AppGUI.TopLayerPanel.TopPanel":
            ...
            if page_name == "src.AppGUI.FileDownloadPanel.FileDownloadsPanel":
        ...
        # Show Menu bar
        self.show_menu("src.AppGUI.MainMenuPanel.MainMenuBar")

        # Closing app
        self.protocol("WM_DELETE_WINDOW", self.close_application)

    def show_top_panel_frame(self):
        print("Showing: " + "src.AppGUI.TopLayerPanel.TopPanel")
        top_panel_frame = self.frames["src.AppGUI.TopLayerPanel.TopPanel"]
        top_panel_frame.grid_remove()
        top_panel_frame.grid(row=1, column=1)

    def show_downloads_panel_frame(self):
        print("Showing: " + "src.AppGUI.FileDownloadPanel.FileDownloadsPanel")
        downloads_panel_frame = self.frames["src.AppGUI.FileDownloadPanel.FileDownloadsPanel"]
